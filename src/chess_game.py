...
# Load new icon
icon = pygame.image.load('../Chess/Pixel Python Logo.png')
...
class ChessGame:
    def __init__(self, __show_legal_moves, __ai_side, fen=None):
        ...
        # White time
        self.white_time = [10, 0]
        # Black time
        self.black_time = [10, 0]
    ...
    def handle_game_state(self):
        if self.api.game_state() == 'check':
            self.check_show = True
            self.blink_enabled = True
            self.checked_king = self.king
        elif self.api.game_state() == 'checkmate':
            self.checkmate = True
            self.checked_king = self.king
        elif self.api.game_state() == 'draw':
            self.draw = True
        else:
...
# game_timer = Timer(TIMER_EVENT, 1000)
...
# Castling
# b = 'r3k2r/pppp1ppp/4pn2/8/1bPPbB2/2N5/PP1QPPPP/R3K2R w - - 0 1'
# # Draw
