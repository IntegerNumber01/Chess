...
class Board:
    ...
    def move(self, uci) -> None:
        ...
        # print('EN PASSANT', self.generator.en_passant)
        # print('TRANSFPRMED', self.to_notation(self.generator.en_passant))
    ...
    def game_state(self) -> str:
        '''
        Returns ``'checkmate', king`` / ``'check', king`` / ``'draw', None``

        ``king`` is the king that has been checkmated or is in check
        '''

        if self.generator.check_for_check(self.board, self.king):
            if self.generator.check_for_checkmate(self.board, self.king):
                return 'checkmate', self.king

            return 'check', self.king
    ...
    def undo_move(self) -> None:
        ...
        self.board, en_passant = self.fen_to_dataframe(self.board_history[-1])
